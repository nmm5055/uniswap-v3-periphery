...
abstract contract FlashLoan is IUniswapV3FlashCallback, PeripheryPayments, PeripheryImmutableState {
    ...
    function initFlash(FlashParams memory params) external {
        ...
        // encodes the two pools for the arbitrage opportunity
        bytes fullPath = abi.encodePacked(params.token0, params.fee1, params.token1, params.token0, params.fee2, params.token1);
        pool.flash(
            params.recipient,
            params.amount0,
            params.amount1,
            abi.encode(FlashCallbackData({amount0: params.amount0, amount1: params.amount1, payer: msg.sender, poolKey: poolKey, path: fullPath}))
        );
    }
